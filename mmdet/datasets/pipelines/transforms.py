...
class Resize(object):
    ...
    def _affine_boxes(self, boxes, mat):
        boxes = np.array(boxes)
        shape = boxes.shape
        boxes = boxes.reshape(-1, 2)
        return np.dot(np.concatenate(
            (boxes, boxes[:, 0:1]*0+1), axis=1), mat.T).reshape(shape)

    def _change(self, results):
        image = results['img']
    ...
    def _resize_areas(self, results):
        for key in results.get('area_fields', []):
            areas = results[key]
            areas = areas * (results['scale_factor'] ** 2)
            results[key] = areas
    ...
    def __call__(self, results):
        ...
        for key in results.get('kpt_fields', []):
            ...
            for i in range(num_kpts):
                gt_vis_flag = kpts[i, :, 2].nonzero()
                gt_kpt = kpts[i][gt_vis_flag]
                x = gt_kpt[:, 0]
                y = gt_kpt[:, 1]
                x1 = x.min()
                y1 = y.min()
                x2 = x.max()
                y2 = y.max()
                area_before.append((x2 - x1) * (y2 - y1))
            area_before = np.array(area_before)
            ...
            for i in range(num_kpts):
                ...
                x2 = x.max()
                y2 = y.max()
                area_now.append((x2 - x1) * (y2 - y1))
            area_now = np.array(area_now)

            results['boxes_ratio'] = area_now / area_before
            results[key] = kpts
        
        for key in results.get('area_fields', []):
            areas = results[key]
            areas = areas * results['boxes_ratio']
            results[key] = areas
        ...
        if 'gt_bboxes' in results:
            ...
            if 'gt_masks_areas' in results:
                results['gt_masks_areas'] = results['gt_masks_areas'][valid_inds]
